...
| ② 연산/통신 breakdown | `attn_step`, `tp_all_reduce` | `role`(in_proj/up_proj/norm/rope/core_attn/out_proj), `ms` |
| ③ 메모리 load/store 오버헤드 | `attn_step`, `attn_step_io` | `bytes_in`/`bytes_out`, `in_shape`/`out_shape` → 유효 대역폭 = bytes/time |

- `role=out_proj` = RowParallelLinear (TP all-reduce 포함). `tp_all_reduce`
